...
namespace DefaultPlanner{
    ...
    void initialize(int preprocess_time_limit, SharedEnvironment* env){
            ...
            prev_states.assign(env->num_of_agents, State());
            next_states.assign(env->num_of_agents, State());
            decided.assign(env->num_of_agents, DCR{-1, DONE::DONE});
            occupied.assign(env->map.size(), false);
            checked.assign(env->num_of_agents, false);
            ids.assign(env->num_of_agents, 0);
            require_guide_path.assign(env->num_of_agents, false);
            dummy_goals.assign(env->num_of_agents, 0);
